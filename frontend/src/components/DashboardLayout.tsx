...
import {
  ...
  HiChevronLeft, HiLogout, HiUser, HiCog, HiChevronDown,
} from 'react-icons/hi';
import {
  Headphones, Video, Clock, BookOpen, WifiOff,
  UserCircle, Settings, LogOut, Library,
} from 'lucide-react';
...
const navGroups: NavGroup[] = [
  {
    labelKey: '',
    items: [
      { href: '/dashboard', labelKey: 'my_library.overview', icon: HiHome },
    ],
  },
  {
    labelKey: 'my_library.group_continue',
    icon: Clock,
    items: [
      { href: '/dashboard/continue-listening', labelKey: 'my_library.continue_listening', icon: Headphones },
      { href: '/dashboard/continue-watching', labelKey: 'my_library.continue_watching', icon: Video },
    ],
  },
  {
    labelKey: 'my_library.group_content',
    icon: BookOpen,
    items: [
      { href: '/dashboard/saved-lessons', labelKey: 'my_library.saved_lessons', icon: HiBookmark },
      { href: '/dashboard/downloaded-audio', labelKey: 'my_library.downloaded_audio', icon: WifiOff },
    ],
  },
  {
    labelKey: 'my_library.group_history',
    icon: Clock,
    items: [
      { href: '/dashboard/listening-history', labelKey: 'my_library.listening_history', icon: Headphones },
      { href: '/dashboard/watch-history', labelKey: 'my_library.watch_history', icon: Video },
    ],
  },
  {
    labelKey: 'my_library.group_settings',
    icon: UserCircle,
    items: [
      { href: '/dashboard/profile', labelKey: 'my_library.profile', icon: HiUser },
      { href: '/dashboard/settings', labelKey: 'my_library.settings', icon: HiCog },
    ],
  },
];
  ...
  return (
    <BackgroundLayout>
      <div className="flex h-screen overflow-hidden">
        ...
        <aside className={`fixed inset-y-0 start-0 z-40 w-64 max-w-[85vw] transform transition-transform duration-300 ease-in-out will-change-transform ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'
        } lg:translate-x-0 lg:rtl:translate-x-0 lg:static lg:inset-auto bg-surface-900/95 border-e border-white/5 flex flex-col backdrop-blur-2xl`}>
          <div className="flex items-center justify-between h-16 px-4 border-b border-white/5 shrink-0">
            <Link to="/dashboard" className="flex items-center gap-2.5">
              ...
              <div>
                <span className="text-sm font-bold text-white block leading-tight">{t('my_library.title')}</span>
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {item.badge && (
                            <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded-full bg-icc-500/20 text-icc-400">{item.badge}</span>
                          )}
                        </Link>
                      );
                    })}
          ...
          <header className="h-16 bg-surface-900/80 border-b border-white/5 flex items-center justify-between px-4 lg:px-6 shrink-0 backdrop-blur-xl">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="hidden sm:block">
                  ...
                  <p className="text-[10px] text-icc-400 font-medium">{t('my_library.subtitle')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={t('my_library.logout')}
              >
                <LogOut className="w-4 h-4" />
                <span className="hidden sm:inline">{t('my_library.logout')}</span>
              </button>
            </div>
          </header>
