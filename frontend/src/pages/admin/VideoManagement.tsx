...
function totalSize(files: VideoResource[]): string {
...
export default function VideoManagement() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<VideoResource | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [previewItem, setPreviewItem] = useState<VideoResource | null>(null);
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      .then(([res, statsRes]) => {
        ...
        setResources(all.filter((r: VideoResource) => r.resourceType === 'VIDEO'));
        const s = statsRes.data as any;
        setStats({
          totalVideo: s.totalVideo || 0,
          totalViews: s.totalViews || 0,
          totalStorage: s.storageByType?.video || s.totalStorage || 0,
        });
      })
      .catch(() => {})
  ...
  useEffect(() => {
    seriesApi.getAll().then(res => {
      setSeriesList(Array.isArray(res.data) ? res.data : res.data?.items ?? []);
    }).catch(() => {});
  }, []);
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!uploadFile) { setUploadError('Please select a video file to upload'); return; }
    ...
    try {
      ...
      fd.append('seriesId', uploadSeriesId || '');
      ...
      setUploadError(err.response?.data?.error || 'Failed to upload video');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleEdit = async () => {
    ...
    try {
      await admin.resources.update(editTarget.id, {
        ...
        seriesId: (editTarget as any).seriesId,
      });
      setEditTarget(null);
      load();
    } catch { /* silent */ }
  ...
  const handlePreview = (item: VideoResource) => {
    setPreviewItem(item);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold truncate">{t('admin.video_management_title')}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <div className="glass-card-dark rounded-xl p-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold">{stats.totalVideo || resources.length}</p>
          </div>
        </div>
        <div className="glass-card-dark rounded-xl p-4 flex items-center gap-4">
