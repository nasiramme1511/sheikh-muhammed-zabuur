...
import { admin, resources, series as seriesApi } from '../../lib/api';
...
export default function RecordingArchive() {
  ...
  const [editForm, setEditForm] = useState<EditForm>({ title: '', description: '', category: 'General', featured: false, seriesId: '' });
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 max-w-lg w-full shadow-2xl">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select value={String(editForm.seriesId ?? '')} onChange={(e) => setEditForm({ ...editForm, seriesId: e.target.value ? Number(e.target.value) : null })} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-icc-500/30 focus:border-icc-500 transition-all">
