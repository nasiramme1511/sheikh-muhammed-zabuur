...
export default function ContinueWatching() {
  ...
  useEffect(() => {
    ...
    historyApi.getWatching().then(res => {
      setItems((res.data || []).filter((h: WatchHistoryItem) => !h.completed && h.progress > 0));
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
        <Video className="w-6 h-6 text-purple-400" /> {t('my_library.continue_watching')}
      </h1>

      {items.length === 0 ? (
        <div className="text-center py-16">
          <Video className="w-16 h-16 text-white/10 mx-auto mb-4" />
          <p className="text-white/60">{t('my_library.no_continue_watching')}</p>
        </div>
      ) : (
