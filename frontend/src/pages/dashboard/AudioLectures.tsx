...
import { Headphones, Play, Clock, Download, Search, ExternalLink } from 'lucide-react';
...
export default function AudioLectures() {
  ...
  useEffect(() => {
    resources.getAll({ resourceType: 'AUDIO', limit: 50 })
    ...
    <div className="space-y-6 pb-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <Headphones className="w-6 h-6 text-icc-400" /> Audio Lectures
        </h1>
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30" />
          <input
            type="text" placeholder="Search audio..."
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-lg text-white/70">No audio lectures found</p>
        </motion.div>
      ) : (
          ...
          {filtered.map((item: any, i: number) => (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-icc-500/20 to-icc-600/10 border border-icc-500/20 flex items-center justify-center shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/40 mt-0.5 truncate">{item.category}</p>
