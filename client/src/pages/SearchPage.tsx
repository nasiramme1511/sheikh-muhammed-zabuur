...
export default function SearchPage() {
      ...
      <div className="space-y-8 animate-pulse mt-8">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((n) => (
              <div key={n} className="bg-dark-800/40 h-56 rounded-2xl border border-white/5 p-4 space-y-3">
  ...
  return (
    <div className="min-h-screen bg-dark-900 pt-24 pb-16 text-white px-4 md:px-8" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative mb-10 text-center">
          ...
          <p className="max-w-xl mx-auto text-sm md:text-base text-white/60 mb-8">
          ...
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto">
            <div className="relative rounded-2xl shadow-sm">
              ...
              <input
                ...
                className={`w-full ${isRTL ? 'pr-12 pl-4' : 'pl-12 pr-4'} py-3.5 text-base sm:text-lg rounded-2xl border border-white/10 bg-dark-800 text-white placeholder:text-white/40 focus:ring-2 focus:ring-emerald-500/50 focus:border-transparent outline-none shadow-sm transition-all duration-300`}
              />
              <button
                type="submit"
        ...
        {!query && !loading && !results && (
          <div className="max-w-2xl mx-auto mt-12 bg-dark-800/30 border border-white/5 rounded-3xl p-8 shadow-sm backdrop-blur-sm text-center">
            ...
            <div className="flex flex-wrap gap-2.5 justify-center">
              {POPULAR_SEARCHES.map((term) => (
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-dark-900 text-sm font-medium hover:bg-emerald-500/10 text-white/60 hover:text-emerald-400 border border-white/5 hover:border-emerald-500/30 hover:scale-105 active:scale-95 transition-all duration-200"
                >
                  {term}
                </button>
              ))}
        ...
        {results && !loading && (
          <div className="space-y-8">
            {/* Results Summary and Tabs Container */}
            <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-white/5 pb-4 gap-4">
              <div className="text-sm font-semibold text-white/60">
                {getResultsCount()} {t('search.results_found')} "{query}"
              </div>

              {/* Premium Filtering Tabs */}
              <div className="flex flex-wrap gap-1 bg-dark-800/80 p-1.5 rounded-2xl border border-white/5">
                {[
                  ...
                  return (
                    ...
                    >
                      <span>{tab.label}</span>
                      <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${isActive ? 'bg-white/20 text-white' : 'bg-dark-900 text-white/40'}`}>
                        {tab.count}
                      </span>
                    </button>
                  );
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-white/50 line-clamp-2 mb-4">
                          {item.description || 'Audio lecture delivered by Sheikh Mohammed Zabuur.'}
                        </p>
                      </div>
                      <div className="flex items-center gap-3 pt-3 border-t border-white/5">
                        <button
                          onClick={() => handlePlayAudio(item)}
                          className="flex-1 py-2 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shadow-md shadow-blue-600/15"
                        >
                        ...
                        <a
                          href={item.url}
                          download
                          onClick={() => resourcesApi.download(item.id)}
                          className="p-2 rounded-xl bg-dark-900 border border-white/5 hover:border-white/20 text-white/60 hover:text-white transition-all"
                  ...
                  {results.videos.map((item) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="glass-card flex flex-col overflow-hidden h-full cursor-pointer group"
                      >
                        ...
                        <div className="p-5 flex flex-col justify-between flex-grow">
                          <div>
                            ...
                            <p className="text-xs text-white/50 line-clamp-2">
                              {item.description || 'Watch video lecture by Sheikh Mohammed Zabuur.'}
                            </p>
                          </div>
                        </div>
                      </motion.div>
                    ...
                    >
                      <div>
                        ...
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div>
                            <h3 className="text-base font-bold text-white leading-snug line-clamp-2 group-hover:text-purple-400 transition-colors">
                              {item.title}
                            </h3>
                          </div>
                        </div>
                        <p className="text-xs text-white/50 line-clamp-2 mb-4">
                          {item.description || 'Download or view PDF lesson material and study files.'}
                        </p>
                      </div>
                      <div className="flex items-center gap-3 pt-3 border-t border-white/5">
                        <button
                          onClick={() => handleOpenPdf(item)}
                          className="flex-1 py-2 px-4 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shadow-md shadow-purple-600/15"
                        >
                      ...
                      >
                        ...
                        <div className="p-5 flex flex-col justify-between flex-grow">
                          <div>
                            ...
                            <h3 className="text-base font-bold text-white mb-2 line-clamp-2 leading-snug group-hover:text-amber-400 transition-colors">
                              {item.title}
                            </h3>
                            <p className="text-xs text-white/50 line-clamp-2">
                              {item.description || 'Past live broadcast replay by Sheikh Mohammed Zabuur.'}
                            </p>
                          </div>
                        </div>
                      </motion.div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {results.telegramChannels.map((ch) => (
                    <a
                      ...
                      className="glass-card p-5 flex items-start gap-4 group hover:border-sky-500/30 transition-all"
                    >
            ...
            {getResultsCount() === 0 && (
              <div className="text-center py-20 bg-dark-800/20 rounded-3xl border border-white/5 backdrop-blur-sm">
        ...
        {activeAudio && (
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="w-full max-w-md bg-dark-800 border border-white/10 rounded-2xl p-6 shadow-2xl text-center"
            >
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-white/5">
                <span className="text-xs text-white/40 uppercase font-semibold tracking-wider">Audio Player</span>
                <button
                  onClick={() => setActiveAudio(null)}
                  className="p-1 rounded-lg hover:bg-white/5 text-white/60 hover:text-white"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
        ...
        {activePdf && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95, y: 20 }}
              onClick={(e) => e.stopPropagation()}
              className="relative w-full max-w-5xl h-[85vh] bg-dark-900 border border-white/10 rounded-2xl overflow-hidden shadow-2xl flex flex-col"
            >
              <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 bg-dark-850">
