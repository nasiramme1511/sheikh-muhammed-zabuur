...
import { Download, Headphones, Video, FileText, Search } from 'lucide-react';
...
export default function DashboardDownloads() {
  ...
  const getIcon = (action: string) => {
    ...
    return <Download className="w-5 h-5 text-gold-400" />;
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-sm text-white/40">{downloads.length} total</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-card-dark p-12 text-center">
          ...
          <p className="text-white/50">No downloads found</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((log) => {
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{meta.title || getLabel(log.action)}</p>
