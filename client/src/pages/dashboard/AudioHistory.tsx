    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="glass-card-dark p-12 text-center">
          ...
          <Link to="/audio" className="btn-icc inline-flex items-center gap-2">
            ...
            return (
              <motion.div key={item.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card-dark p-4 group">
                <div className="flex items-center gap-3">
                  ...
                  {item.lesson?.slug && (
                    <Link to={`/lessons/${item.lesson.slug}`} className="p-2 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-emerald-500/10 text-white/30 hover:text-emerald-400 transition-all">
